...
//short
public class TroveShortObjectMapTest {
    ...
    @Test
    public void test() {
        Gson gson = new GsonBuilder().registerTypeAdapter(new TypeToken<TShortObjectMap>(){}.getType(), new TroveShortObjectMapTypeAdapter<>()).create();
        
        short[] f = new short[300];
        for (int i = 0; i < f.length; i++) {
            f[i] = (short) (rand.nextDouble() * 127);
        }
        TShortObjectMap<Filler> list = new TShortObjectHashMap<>();
        for (int i = 0; i < f.length; i++) {
            list.put(f[i], new Filler());
        }
        
        String s = gson.toJson(list, new TypeToken<TShortObjectMap<Filler>>(){}.getType());
        System.out.println(s);
        TShortObjectMap<Filler> after = gson.fromJson(s, new TypeToken<TShortObjectMap<Filler>>(){}.getType());
