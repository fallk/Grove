...
//char
public class TroveCharObjectMapTest {
    ...
    @Test
    public void test() {
        Gson gson = new GsonBuilder().registerTypeAdapter(new TypeToken<TCharObjectMap>(){}.getType(), new TroveCharObjectMapTypeAdapter<>()).create();
        
        char[] f = new char[300];
        for (int i = 0; i < f.length; i++) {
            f[i] = (char) (rand.nextDouble() * 127);
        }
        TCharObjectMap<Filler> list = new TCharObjectHashMap<>();
        for (int i = 0; i < f.length; i++) {
            list.put(f[i], new Filler());
        }
        
        String s = gson.toJson(list, new TypeToken<TCharObjectMap<Filler>>(){}.getType());
        System.out.println(s);
        TCharObjectMap<Filler> after = gson.fromJson(s, new TypeToken<TCharObjectMap<Filler>>(){}.getType());
