...
//byte
public class TroveByteObjectMapTest {
    ...
    @Test
    public void test() {
        Gson gson = new GsonBuilder().registerTypeAdapter(new TypeToken<TByteObjectMap>(){}.getType(), new TroveByteObjectMapTypeAdapter<>()).create();
        
        byte[] f = new byte[300];
        for (int i = 0; i < f.length; i++) {
            f[i] = (byte) (rand.nextDouble() * 127);
        }
        TByteObjectMap<Filler> list = new TByteObjectHashMap<>();
        for (int i = 0; i < f.length; i++) {
            list.put(f[i], new Filler());
        }
        
        String s = gson.toJson(list, new TypeToken<TByteObjectMap<Filler>>(){}.getType());
        System.out.println(s);
        TByteObjectMap<Filler> after = gson.fromJson(s, new TypeToken<TByteObjectMap<Filler>>(){}.getType());
