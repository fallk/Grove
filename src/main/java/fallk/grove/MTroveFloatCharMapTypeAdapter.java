...
//float,char
public final class MTroveFloatCharMapTypeAdapter implements JsonSerializer<TFloatCharMap>, JsonDeserializer<TFloatCharMap> {
    ...
    public TFloatCharMap deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        ...
        for (Entry<String, JsonElement> el : jmap.entrySet()) {
            map.put(Float.parseFloat(el.getKey()), Char.parseChar(el.getValue().getAsString()));
        }
        return map;
    }
    ...
    public JsonElement serialize(TFloatCharMap src, Type typeOfSrc, JsonSerializationContext context) {
        ...
        for (float key : keys) {
            jmap.addProperty(Float.toString(key), Char.toString(src.get(key)));
        }
