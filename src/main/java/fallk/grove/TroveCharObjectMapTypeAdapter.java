...
//char
public final class TroveCharObjectMapTypeAdapter<V> implements JsonSerializer<TCharObjectMap<V>>, JsonDeserializer<TCharObjectMap<V>> {
    // guava says to implement this differently, check doc if this dosnt work

    //@SuppressWarnings("unchecked")
    public TCharObjectMap<V> deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        JsonObject jmap = json.getAsJsonObject();
        TCharObjectMap<V> map = new TCharObjectHashMap<>(jmap.size());
...
//        Type genericType = typeToken.getType(); // or getRawType() to return Class<? super T>
        ...
        for (Entry<String, JsonElement> el : jmap.entrySet()) {
            ...
            map.put(Char.parseChar(el.getKey()), context.deserialize(el.getValue(), genericType));
        }
        return map;
    }
    ...
    public JsonElement serialize(TCharObjectMap<V> src, Type typeOfSrc, JsonSerializationContext context) {
        ...
        for (char key : keys) {
            jmap.add(Char.toString(key), context.serialize(src.get(key), getGenericType(typeOfSrc)));
        }
        return jmap;
    }
}