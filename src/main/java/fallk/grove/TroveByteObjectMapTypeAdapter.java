...
//byte
public final class TroveByteObjectMapTypeAdapter<V> implements JsonSerializer<TByteObjectMap<V>>, JsonDeserializer<TByteObjectMap<V>> {
    // guava says to implement this differently, check doc if this dosnt work

    //@SuppressWarnings("unchecked")
    public TByteObjectMap<V> deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        JsonObject jmap = json.getAsJsonObject();
        TByteObjectMap<V> map = new TByteObjectHashMap<>(jmap.size());
...
//        Type genericType = typeToken.getType(); // or getRawType() to return Class<? super T>
    ...
    public JsonElement serialize(TByteObjectMap<V> src, Type typeOfSrc, JsonSerializationContext context) {
        ...
        for (byte key : keys) {
            jmap.add(Byte.toString(key), context.serialize(src.get(key), getGenericType(typeOfSrc)));
        }
        return jmap;
    }
}