...
//int
public final class TroveIntObjectMapTypeAdapter<V> implements JsonSerializer<TIntObjectMap<V>>, JsonDeserializer<TIntObjectMap<V>> {
    // guava says to implement this differently, check doc if this dosnt work

    //@SuppressWarnings("unchecked")
    public TIntObjectMap<V> deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        JsonObject jmap = json.getAsJsonObject();
        TIntObjectMap<V> map = new TIntObjectHashMap<>(jmap.size());
...
//        Type genericType = typeToken.getType(); // or getRawType() to return Class<? super T>
        ...
        for (Entry<String, JsonElement> el : jmap.entrySet()) {
            ...
            map.put(Int.parseInt(el.getKey()), context.deserialize(el.getValue(), genericType));
        }
        return map;
    }
    ...
    public JsonElement serialize(TIntObjectMap<V> src, Type typeOfSrc, JsonSerializationContext context) {
        ...
        for (int key : keys) {
            jmap.add(Int.toString(key), context.serialize(src.get(key), getGenericType(typeOfSrc)));
        }
        return jmap;
    }
}