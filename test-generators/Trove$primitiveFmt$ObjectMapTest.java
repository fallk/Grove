...
//$primitive$
public class Trove$primitiveFmt$ObjectMapTest {
    ...
    @Test
    public void test() {
        Gson gson = new GsonBuilder().registerTypeAdapter(new TypeToken<T$primitiveFmt$ObjectMap>(){}.getType(), new Trove$primitiveFmt$ObjectMapTypeAdapter<>()).create();
        
        $primitive$[] f = new $primitive$[300];
        for (int i = 0; i < f.length; i++) {
            f[i] = ($primitive$) (rand.nextDouble() * 127);
        }
        T$primitiveFmt$ObjectMap<Filler> list = new T$primitiveFmt$ObjectHashMap<>();
        for (int i = 0; i < f.length; i++) {
            list.put(f[i], new Filler());
        }
        
        String s = gson.toJson(list, new TypeToken<T$primitiveFmt$ObjectMap<Filler>>(){}.getType());
        System.out.println(s);
        T$primitiveFmt$ObjectMap<Filler> after = gson.fromJson(s, new TypeToken<T$primitiveFmt$ObjectMap<Filler>>(){}.getType());

        $primitiveFmt$Asserter.arrayEquals(list.keys(), after.keys());
        $primitiveFmt$Asserter.arrayEquals(list.values(), after.values());
    }
}
